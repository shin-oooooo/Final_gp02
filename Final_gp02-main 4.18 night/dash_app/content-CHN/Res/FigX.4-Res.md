...
本文与侧栏渲染模板对齐：`dash_app/render/explain/sidebar_right/figx4.py` · `build_figx4_explain_body(...)` 读取 `snap_json["phase2"]` + `DefensePolicyConfig` 并把占位符注入到 `FigX.4-Res.md`。因此本文必须保留占位符：`—`、`—`、`—`、`—`、``、`0.00000000`、`0.00000000`、`0.00000000`、`0.00000000`、`否`、`否`、`—`、`0.45`、`0.7`、`0`、以及自动展开块 `- **平均 NLL**：—` / `- **DM p 值（对 Naive）**：—` / `- **名义 95% 实证覆盖率**：—` / `—`。

占位符：`**{n_symbols}`** · `**0.00000000`** · `**0.00000000`** · `**0.00000000`** · `**否**` · `**0.45**` · `**0.7**` · `**0**`。
...
- **生命周期**：`—`～`—`（实际切片由 `resolve_dynamic_train_test_windows(...)` 在 `research/pipeline.py` 中解析并写入 `phase0.meta.resolved_windows`）。  
- **频率**：交易日（日频）。  
...
- `policy.credibility_baseline_jsd_scale = 6.0000`：决定基准项曲率；α 越大，仪表盘数值在同一 `jsd_triangle_mean` 下越低。  
- `policy.credibility_penalty_jsd_scale = 0.1200` 与 `policy.credibility_penalty_cap = 0.3500`：只在 `否` 为“是”时生效；决定扣分斜率与最大扣分。  
- `policy.credibility_score_min/max = -0.5000 / 1.0000`：决定输出 clip 区间；若设置异常导致 `min≥max`，代码会回退到 `[-0.5, 1.0]`。  
- `policy.tau_l2=0.45`、`policy.tau_l1=0.7`：决定红/黄/绿阈值，并通过 `resolve_defense_level` 影响 `defense_level=0`。
...
记 \overline{\mathrm{JSD}}_{\triangle}=`phase2.jsd_triangle_mean`。本运行：`**0.00000000**`。

- 基准项：`credibility_base_jsd` = `**0.00000000**`，其中 α = `**6.0000**`。  
- 覆盖惩罚：`credibility_coverage_penalty` = `**0.00000000**`，仅当 `density_test_failed`=`**否**` 时启用；β = `**0.1200**`，上限 γ = `**0.3500**`。  
- clip：区间 `**-0.5000**`～`**1.0000**`，最终 `credibility_score` = `**0.00000000**`（亦即 `consistency_score` = `**0.00000000**`）。
...
阈值与状态：**τ_L2=`0.45`**，**τ_L1=`0.7`**；`**defense_level**` = `**0**`。
...
**占位**：`**{n_symbols}`** · `**0.00000000`** · `**0.45`** · `**0.7`** · `**0**`
