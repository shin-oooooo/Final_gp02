...
`0.00000000`、`0.00000000`、`0.00000000`、`0.00000000`、`0.00000000`、`0.00000000`、`5`、`2.0000`、`1.0000e-09`、`0.00000000`、`0`、`—`，以及通用字段 `—`、`—`、`—`、`—`、``、`0.00000000`、`否`（来自快照 phase2）。

> **窗口口径统一**：从本版起，JSD 应力的滚动窗口 **W** 与 FigX.6 语义–数值滚动余弦共用同一策略参数 `DefensePolicyConfig.semantic_cosine_window`（侧栏「W 计算滚动窗口长度」，默认 **5 日**），并同时决定「训练滚动基线窗」与「测试窗告警滚动窗」。图上白实线 **滚动三角均值（W=5，告警口径）** 即红色首次越线竖线的真正触发序列；原先独立的 `n_jsd` 参数已废弃。

占位符：`**0.00000000**` · `**0.00000000**` · `**否**`
...
Phase2 strict OOS：对每个测试日 t，仅用 I_{t-1} 拟合各模型的 μ/σ
  ...
  → Dash：fig_defense_jsd_stress_timeseries（日度三角 + 白色滚动三角均值（W=5）+ 阈值线 + 首次 breach 竖线）
```
...
| 变量 | 字段 | 注入值 |
|---|---|---|
| 三角均值（跨日均） | `phase2.jsd_triangle_mean` | `**0.00000000**` |
| 三边最大均值 | `phase2.jsd_triangle_max` | `**0.00000000**` |
| 动态阈值 | `k_jsd×max(baseline,eps)` | `**0.00000000**` |
| 是否触发 | `phase2.jsd_stress` | `**否**` |
| 日度尾段（审计） | `phase2.test_daily_triangle_jsd_mean` | `len=0`，tail=`—` |
...
2. 手工复算阈值 \(\tau=k\cdot\max(baseline,\varepsilon)\)，与 `0.00000000` 一致。
3. 复算 breach：若存在任一窗口滚动均值超过阈值，则 `否` 应为"是"；否则为"否"（样本不足时按回退判定）。
...
**If** `滚动三角 JSD 应力触发 = 否` **Then**
`FigX.5: 于{jsd_alarm_date}，滚动三角JSD均值={jsd_mean_at_breach}第一次超过τ=0.00000000→{jsd_alarm_date}为首次预警日，防御等级切换至 Level 2`
`severity: danger`

**Else**
`FigX.5: 滚动三角 JSD 未超过 τ=0.00000000；当前变量对防御等级切换无直接影响`
`severity: success`
