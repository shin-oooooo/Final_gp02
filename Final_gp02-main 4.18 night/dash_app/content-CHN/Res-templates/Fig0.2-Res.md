...
本文与 Phase 0 主列热力图对齐：`figure_title="Figure0.2"`（`dash_app/app.py` 调用 `fig_correlation_heatmap`）。占位符 `**{train_start}**` · `**{train_end}**` · `**{cross_threshold}**` · `**{n_symbols}**` 可由 Caption/`_fmt_vars` 或快照解析注入。
...
**占位**：`**{train_start}`** · `**{train_end}`** · `**{cross_threshold}`** · `**{n_symbols}`**