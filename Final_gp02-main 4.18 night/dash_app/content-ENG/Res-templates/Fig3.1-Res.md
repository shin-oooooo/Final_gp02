...
Placeholder: `**{n_symbols}`** · `**{defense_level}`** · `**{objective_name}`** · (If required to be linked with Phase 2 drop-down) `**{p2_selected_symbol}**`
...
Placeholder context: `**{defense_level}`**, `**{objective_name}`** and `**objective-banner**` (optimization goal description) align the sidebar `**DefensePolicyConfig**`; this figure **only exposes the original values of μ̂/σ̂** and does not do any rescaling of the **objective function**.

---

**Placeholder**: `**{n_symbols}`** · `**{defense_level}`** · `**{objective_name}`** · `**{p2_selected_symbol}**`