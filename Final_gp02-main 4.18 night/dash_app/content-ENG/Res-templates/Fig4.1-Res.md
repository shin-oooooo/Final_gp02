...
| --------- | ------------------------------------------ |
| Reference day definition | `**{research_failure_ref_label}`** |
| Is there an effective lead time | `**{early_warning_valid}**` |
| Lead time summary | `**{early_warning_signals}**` |
| Detailed lead time | `{early_warning_leads_md}` |
| JSD: Stress or not | `**{jsd_stress}**` (or `{jsd_stress_no}` if not) |
| Cosine: Whether to diverge | `**{lb_cos}**` (if not, you can see `{cosine_ge_zero}`) |
...
**If** `Alert valid = {early_warning_valid}` **Then**
`Fig4.1: Early warning is valid ({early_warning_signals}), reference day definition: {research_failure_ref_label}`
`severity: success`

**Else**
`Fig4.1: No effective advance is observed (need to check the t_ref definition or whether the signal trigger is too late), refer to the day definition: {research_failure_ref_label}`
`severity: warn`