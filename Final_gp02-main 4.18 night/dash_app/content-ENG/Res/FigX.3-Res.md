...
This article is aligned with the sidebar rendering template: `dash_app/render/explain/sidebar_right/figx3.py` · `build_figx3_explain_body(...)` will inject placeholders: `0.0000`, `0.5000`, `32.00%`, `-0.0800`, and expand block `*无未通过 ADF 的标的*` / `*无高波动资产*` / `*无低自相关资产*`.

Placeholder: `**0.0000**` · `**32.00%**` · `**-0.0800**`
...
- Context scalar: structural entropy `**0.0000**` and threshold `τ_h1=**0.5000**` (used to illustrate "whether the macro environment has been biased towards Level1")

### 3.2 Variable mapping

- **Enter X**:
  - `diagnostics[*].stationary_returns/basic_logic_failure/vol_ann/ac1`
  - `policy.tau_vol_melt=32.00%`
  - `policy.tau_return_ac1=-0.0800`
- **Output Y (sidebar expander)**:
  - `*无未通过 ADF 的标的*`: ADF uncleared asset list
  - `*无高波动资产*`: List of highly volatile assets
  - `*无低自相关资产*`: Low AC1 asset list
...
2. Use the `diagnostic_failed_adf` caliber to recalculate the ADF failed asset list, which should be consistent with `*无未通过 ADF 的标的*`.
3. Recalculate the high volatility/low AC1 list with thresholds, which should be consistent with `*无高波动资产*`/`*无低自相关资产*`.
