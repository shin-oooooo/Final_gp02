...
- **Default parameters (v2)**: α=0.7, β=0.6, γ=0.5, H=7 calendar days; `normalize_kernel=True`, `soft_clip="tanh"`, `include_today_in_memory=False`. In the "almost all negative news + P+B ≈ −0.3" scenario, S_t stabilizes at [−0.9, −0.2] and shows visible fluctuations with news density/polarity, rather than stacking to −1.
...
- **Threshold**: \tau_{S,low}=\texttt{policy.tau_s_low}=**-0.200** (When the consistency is high enough, it is still required that the emotion is not lower than the threshold before entering Level 0).
- **Half-life**: H=\texttt{policy.sentimenthalflifedays}=**7.00** calendar days.
- **Constant bias**: `penalty`=**+0.0000**, `severity_boost`=**+0.0000**; full-window VADER mean (reference value)=**+0.0000**; pooled headline number=**0**.
...
- **Target observation variable Y (runtime injection)**:
  - `S_t` summary: min=`**—`**, max=`**—`**, last=`**—**`
  - "Coverage days/caliber": `**—**` (see sidebar builder for specific caliber)
...
- **Half-life H is a hyperparameter**: too short → the historical memory weight decays rapidly, and S_t is close to the noise of VADER on the day; too long → the impact of the event lasts too long and is not robust to short noise. The default H=7 calendar days is a neutral choice; adjustable in event dense/sparse windows via `DefensePolicyConfig.sentiment_halflife_days`.
...
`FigX.1: The test window sentiment has an extreme low point st_min=— < τ_S_low=-0.200, which may still prevent entry to Level 0 when the consistency is high`
`severity: warn`

**Else**
`FigX.1: st_min=— ≥ τ_S_low=-0.200; the current variables have no direct impact on defense level switching`
`severity: success`