...
This article is aligned with the Phase 0 main column heatmap: `figure_title="Figure0.2"` (`dash_app/app.py` calls `fig_correlation_heatmap`). Placeholders `**—**` · `**—**` · `**{cross_threshold}**` · `**{n_symbols}**` can be injected by Caption/`_fmt_vars` or snapshot parsing.
...
**Placeholder**: `**—`** · `**—`** · `**{cross_threshold}`** · `**{n_symbols}`**