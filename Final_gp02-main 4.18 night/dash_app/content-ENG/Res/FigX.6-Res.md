...
This article is aligned with the sidebar rendering template: `dash_app/render/explain/sidebar_right/figx6.py` · `build_figx6_explain_body(...)` Placeholders will be injected: `5`, `0.00000000`, `否`, `否`, `0.00000000`, `-0.0800`, `否`, `否`, `—`, and general fields `—`, `—`, `—`, `—`, ``, `0.00000000`.

Placeholders: `**0.00000000`** · `**5**` · `**否**` · `**否**`
...
- Parameter: `policy.semantic_cosine_window = 5`

### 3.2 Variable mapping (runtime injection)

| Variable | Field | Inject value |
| ---------- | ----------------------------------------------- | -------------------------- |
| latest cosine | `phase2.cosine_semantic_numeric` | `**0.00000000**` |
| Whether the calculation is successful | `phase2.semantic_numeric_cosine_computed` | `**否**` |
| Logic break (cosine) | `phase2.logic_break_semantic_cosine_negative` | `**否**` |
| Logic break (total) | `phase2.logic_break` | `**否**` |
| S_t tail segment (audit) | `meta.test_sentiment_st` | `—` |
...
2. Use the same window W=5 to recalculate the rolling cosine sequence:
...
**If** `Semantic departure trigger = 否` **Then**
...
**Else**
`FigX.6: Semantics – No negative values appear in numerical rolling cosine (last value =0.00000000); the current variable has no direct impact on defense level switching`
`severity: success`
