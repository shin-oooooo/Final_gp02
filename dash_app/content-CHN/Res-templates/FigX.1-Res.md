...
- **阈值**：\tau_{S,low}=\texttt{policy.tau_s_low}=**{tau_s_low}**（当一致性已足够高时，仍要求情绪不低于该阈值才允许进入 Level 0）。
- **半衰期**：H=\texttt{policy.sentimenthalflifedays}=**{sentiment_halflife_days}** 日历天。
- **常量偏置**：`penalty`=**{sentiment_penalty}**、`severity_boost`=**{sentiment_severity_boost}**；全窗 VADER 均值（参考值）=**{sentiment_vader_avg}**；入池 headline 数=**{sentiment_n_headlines}**。
...
- **目标观察变量 Y（运行时注入）**：
  - `S_t` 摘要：min=`**{st_min}`**、max=`**{st_max}`**、last=`**{st_last}**`
  - “覆盖天数/口径”：`**{st_news_days}**`（具体口径见 sidebar builder）
...
| 3   | 指数核卷积生成 S_t | M_i + trading index + `halflife_days` + `penalty` + `severity_boost` | `st_test`                                             | S_t=\text{clip}(\alpha V_t+\beta\mathcal{H}_t+P+B,-1,+1)，核距离按日历天     | `vader_st_series_kernel_smoothed_from_detail`, `research/pipeline.py`     |
...
`FigX.1: 测试窗情绪出现极端低点 st_min={st_min} < τ_S_low={tau_s_low}，在一致性较高时仍可能阻止进入 Level 0`
`severity: warn`
