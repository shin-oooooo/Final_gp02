...
本文与侧栏渲染模板对齐：`dash_app/render/explain/sidebar_right/figx6.py` · `build_figx6_explain_body(...)` 会注入占位符：`{cos_w}`、`{cosine}`、`{lb_cos}`、`{sem_cos_computed}`、`{train_return_ac1}`、`{tau_ac1}`、`{logic_break_ac1}`、`{logic_break_total}`、`{st_path_tail_md}`，以及通用字段 `{train_start}`、`{train_end}`、`{test_start}`、`{test_end}`、`{p0_symbols_csv}`、`{credibility}`。

占位符：`**{cosine}`** · `**{cos_w}**` · `**{lb_cos}**` · `**{logic_break_total}**`
...
- 参数：`policy.semantic_cosine_window = {cos_w}`

### 3.2 变量映射（运行时注入）

| 变量         | 字段                                            | 注入值                       |
| ---------- | --------------------------------------------- | ------------------------- |
| 最新余弦       | `phase2.cosine_semantic_numeric`              | `**{cosine}**`            |
| 是否计算成功     | `phase2.semantic_numeric_cosine_computed`     | `**{sem_cos_computed}**`  |
| 逻辑断裂（余弦）   | `phase2.logic_break_semantic_cosine_negative` | `**{lb_cos}**`            |
| 逻辑断裂（总）    | `phase2.logic_break`                          | `**{logic_break_total}**` |
| S_t 尾段（审计） | `meta.test_sentiment_st`                      | `{st_path_tail_md}`       |
