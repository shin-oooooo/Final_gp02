...
占位符：`**{n_symbols}`** · `**{defense_level}`** · `**{objective_name}`** ·（若需与 Phase 2 下拉联动）`**{p2_selected_symbol}**`
...
占位符语境：`**{defense_level}`**、`**{objective_name}`** 与 `**objective-banner**`（优化目标叙述）对齐侧栏 `**DefensePolicyConfig**`；本图 **只暴露 μ̂／σ̂ 原始数值**，不做 **目标函数** 的任何重标度。

---

**占位**：`**{n_symbols}`** · `**{defense_level}`** · `**{objective_name}`** · `**{p2_selected_symbol}**`