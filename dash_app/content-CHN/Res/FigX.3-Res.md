...
本文与侧栏渲染模板对齐：`dash_app/render/explain/sidebar_right/figx3.py` · `build_figx3_explain_body(...)` 会注入占位符：`0.0000`、`0.5000`、`32.00%`、`-0.0800`，以及展开块 `*无未通过 ADF 的标的*` / `*无高波动资产*` / `*无低自相关资产*`。

占位符：`**0.0000**` · `**32.00%**` · `**-0.0800**`
...
- 上下文标量：结构熵 `**0.0000**` 与阈值 `τ_h1=**0.5000**`（用于说明“宏观环境是否已经偏向 Level1”）
...
2. 用 `diagnostic_failed_adf` 口径复算 ADF 失败资产清单，应与 `*无未通过 ADF 的标的*` 一致。
3. 用阈值复算高波动/低 AC1 清单，应与 `*无高波动资产*`/`*无低自相关资产*` 一致。
