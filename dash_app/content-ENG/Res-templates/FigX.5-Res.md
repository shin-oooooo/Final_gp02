...
Phase2 strict OOS: for each test day t, fit each model's μ/σ using only I_{t-1}
  ...
  → Dash: fig_defense_jsd_stress_timeseries (daily triangle + white rolling triangle mean (W={cos_w}) + threshold line + first-breach vertical line)
```
...
| ------------------------------ | ------------------------------------- | ------------------------------------------------------------ |
| Triangle mean (cross-day mean) | `phase2.jsd_triangle_mean`            | `**{jsd_triangle_mean}`**                                    |
| Max-of-three-edges mean        | `phase2.jsd_triangle_max`             | `**{jsd_triangle_max}**`                                     |
| Dynamic threshold              | `k_jsd×max(baseline,eps)`             | `**{jsd_stress_dyn_thr}**`                                   |
| Triggered?                     | `phase2.jsd_stress`                   | `**{jsd_stress}**`                                           |
| Daily tail (audit)             | `phase2.test_daily_triangle_jsd_mean` | `len={test_daily_tri_len}`, tail=`{test_daily_tri_tail_csv}` |
...
2. Re-compute the threshold by hand: \tau=k\cdot\max(baseline,\varepsilon), and check it matches `{jsd_stress_dyn_thr}`.
3. Re-compute the breach: if any rolling-window mean exceeds the threshold, `{jsd_stress}` should be "Yes"; otherwise "No" (apply the fallback rule when samples are insufficient).
...
**If** `rolling triangle-JSD stress trigger = {jsd_stress}` **Then**
`FigX.5: on {jsd_alarm_date}, rolling triangle-JSD mean = {jsd_mean_at_breach} first exceeded τ = {jsd_stress_dyn_thr} → {jsd_alarm_date} is the first alarm day; defense level switches to Level 2`
`severity: danger`

**Else**
`FigX.5: rolling triangle JSD does not exceed τ={jsd_stress_dyn_thr}; this variable has no direct effect on defense-level transitions`
`severity: success`